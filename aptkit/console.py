...
class ConsoleClient:
    ...
    def _on_exception(self, error):
        ...
        try:
            ...
            msg = "%s %s\n\n%s" % ("ERROR:", "You are not allowed to perform this action.",
                                   error.get_dbus_message())
        except dbus.DBusException:
            msg = "%s %s - %s" % ("ERROR:", error.get_dbus_name(),
                                  error.get_dbus_message())
        except:
            msg = str(error)
        self._loop.quit()
    ...
    def _show_changes(self):
        ...
        if self._transaction.download:
            print("Need to get %sB of archives." %
                  client.get_size_string(self._transaction.download))
        if self._transaction.space > 0:
            print("After this operation, %sB of additional disk space "
                    "will be used." %
...
def main():
    ...
    parser.add_option("", "--fix-depends", default="",
                      ...
                      help="Try to resolve broken dependencies. "
    ...
    parser.add_option("", "--fix-install", default="",
                      ...
                      help="Try to finish a previous incompleted "
                             "installation")
    ...
    con._progress_id = GLib.timeout_add(250, con._update_custom_progress, "Waiting for authentication")
